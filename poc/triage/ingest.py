...
_CELL_RE: Final = re.compile(r"<c[^>]*>.*?</c>|<c[^>]*/>", re.S)
_ROW_RE: Final = re.compile(r"<row[^>]*>(.*?)</row>", re.S)
_SI_RE: Final = re.compile(r"<si>(.*?)</si>", re.S)
_TAG_RE: Final = re.compile(r"<[^>]+>")
_TYPE_RE: Final = re.compile(r't="([^"]+)"')
_VALUE_RE: Final = re.compile(r"<v>(.*?)</v>", re.S)
_INLINE_RE: Final = re.compile(r"<is>(.*?)</is>", re.S)

_BR_RE: Final = re.compile(r"<br\s*/?>", re.I)
