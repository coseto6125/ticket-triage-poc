...
欄位順序就是要求模型判斷的順序，reason 放在最前面讓模型先寫下對這封工單的理解，
再照這個理解填後面的標籤。
"""
...
def response_schema() -> dict[str, Any]:
    ...
    properties: dict[str, Any] = {
        "reason": {
            "type": "string",
            "maxLength": 200,
            "description": "用一句繁體中文說明寄件者是誰、這封工單要什麼。只寫一句，這句會被記錄為分流依據。",
        },
        "disposition": {
            "type": "string",
        ...
        "scenarios": {
            ...
            "description": (
                "第三層：這封工單屬於哪些服務情境，可複選。一封工單同時問兩件不同的事就填兩個。"
                "第一層填 reject 時，只填該封工單屬於的 reject 情境。"
            ),
        },
        "is_complaint": {
            "type": "boolean",
            "description": (
                "這封工單是否針對本公司已經發生的作為或服務表達不滿、指責，或要求道歉與補償。"
        ...
        "residual_pii": {
            ...
            "description": (
                "這封工單的文字裡仍然看得到的個人資料原文片段（姓名、電話、證號、帳號、地址）。沒有就給空陣列。"
            ),
        },
        "confidence": {
            "type": "number",
            ...
            "description": (
                ...
                "評的是「這封工單有沒有被分到對的情境」，不是「這個問題好不好回答」："
                "一封訴求很清楚但需要查很多資料才能回覆的工單，分類信心仍然應該很高。"
