...
代價：LLM 的輸出不穩定，`temperature` 壓到 0.1 之後同一封信重跑仍可能得到不同的情境標籤。PoC 的處置是把六條分流規則全部放在程式端，模型只提供其中一項輸入。模型抖動會影響情境標籤，動不到「涉及發票一律轉真人」這種結論。
...
範圍限制（建議值）：試營運先跑四週，只開 `template` 加 `human` 這個組合（先送範本並告知會轉專人，工單仍進人工），以及 `ack_only`。`template` 加 `auto`（送出範本並結案）在 Gate 1 不開。這樣首次回覆時間可以先降下來，而任何一封信都還是有人看過。
...
| 情境怎麼分組 | 用「同一類工具查得到」分組，而且一封信可以同時屬於多個情境 | 6 個情境群組 |
| 情境要多細 | 客服軌保留細分，非客服軌合併成一個 | 17 個服務情境 |
